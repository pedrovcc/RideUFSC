...
import 'package:boilerplate_flutter/modules/core/history/components/history_card.dart';
import 'package:boilerplate_flutter/modules/core/history/components/history_empty.dart';
...
class HistoryListState extends State<HistoryList> {
  ...
  Widget build(BuildContext context) {
    return (widget.rideList.length > 0 && getRideAllocatedInList(widget.rideList))
              ...
              children: [
                Padding(
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, index) => getRideAllocated(widget.rideList[index])
                        ? HistoryCard(
  ...
  bool getRideAllocated(Ride ride) {
    UserModel user = widget.user;
    bool isAllocated = false;

    ride.travelers?.forEach((traveler) {
      if (traveler.id == user.uid) {
        isAllocated = true;
      }
    });
  ...
  bool getRideAllocatedInList(List<Ride> rideList) {
    UserModel user = widget.user;
    bool isAllocated = false;
